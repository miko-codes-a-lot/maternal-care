...
fun ConditionStatusUI(
    ...
    val dateCreatedState = remember { mutableStateOf(userCondition?.createdAt ?: "") }
    Column(
        modifier = Modifier
        ...
        DateCreateContainer(
            label = "Date Created",
            dateValue = dateCreatedState.value,
            onDateChange = { newDate -> dateCreatedState.value = newDate }
        )
        NutritionalStatus(
...
fun ButtonSaveStatus(
    ...
    isNormal: Boolean,
    isCritical: Boolean,
    dateCreatedState: MutableState<String>,
) {
    ...
    Button(
        onClick = {
            val userConditionStatus  = UserConditionDto(
                ...
                createdAt = dateCreatedState.value
            )
            scope.launch {
                try {
